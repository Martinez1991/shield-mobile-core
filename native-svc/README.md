...
Passes: `flatten` (control-flow flattening) is implemented; `mba`, `opaque`,
`strings` are declared in the contract and return "not implemented yet" (exit 3)
until built — never a silent no-op.
